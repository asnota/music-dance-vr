...
def axis_angle_to_quaternion(axis_angle):
    ...
    q = torch.from_numpy(q).to(axis_angle.device).type_as(axis_angle)

    return q.reshape(*orig_shape[:-1], 4)


def quaternion_apply(quaternion, vec):
    """
    Rotate vector(s) using quaternion(s).
    ...
    rotated = vec + q_w * t + torch.cross(q_xyz, t, dim=-1)
...
def skeleton_render(
    ...
    if sound:
        ...
        if stitch:
            ...
            outname = os.path.join(
                out,
                f"{epoch}_{os.path.splitext(os.path.basename(name))[0]}.mp4",
            )

        if render:
            os.system(
                f"ffmpeg -loglevel error -stream_loop 0 -y "
                f"-i {gifname} -i {audioname} "
                f"-shortest -c:v libx264 -crf 26 "
                f"-c:a aac -q:a 4 {outname}"
            )
...
class SMPLSkeleton:
    def __init__(self, device=None):
        ...
        self._offsets = torch.tensor(offsets, dtype=torch.float32).to(device)
        ...
        for i in range(self._offsets.shape[0]):
            ...
            if self._parents[i] == -1:
                ...
                if self._has_children[i]:
                    joint_rotation = quaternion_multiply(
                        rotations_world[parent],
                        rotations[:, :, i],
                    )

                    rotations_world.append(joint_rotation)
