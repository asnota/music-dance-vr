...
def axis_angle_to_quaternion(axis_angle):
    ...
    q = R.from_rotvec(aa).as_quat()
    q = torch.tensor(q, dtype=axis_angle.dtype, device=axis_angle.device)
    return q.reshape(*orig_shape[:-1], 4)


def quaternion_to_axis_angle(quaternion):
    orig_shape = quaternion.shape
    q = quaternion.reshape(-1, 4).detach().cpu().numpy()
    aa = R.from_quat(q).as_rotvec()
    aa = torch.tensor(aa, dtype=quaternion.dtype, device=quaternion.device)
    return aa.reshape(*orig_shape[:-1], 3)


def quaternion_multiply(q1, q2):
    orig_shape = q1.shape
    r1 = R.from_quat(q1.reshape(-1, 4).detach().cpu().numpy())
    r2 = R.from_quat(q2.reshape(-1, 4).detach().cpu().numpy())
    q = (r1 * r2).as_quat()
    q = torch.tensor(q, dtype=q1.dtype, device=q1.device)
    return q.reshape(orig_shape)



def quaternion_apply(q, pts):
    r = R.from_quat(q.reshape(-1, 4).detach().cpu().numpy())
    out = r.apply(pts.reshape(-1, 3).detach().cpu().numpy())
    out = torch.tensor(out, dtype=pts.dtype, device=pts.device)
    ...
    def render_sample(
        ...
        if mode == "long":
            ...
            if fk_out is not None:
                ...
                base = os.path.splitext(os.path.basename(name[0]))[0]
                base = "_".join(base.split("_")[:-1])

                if base == "":
                    base = "motion"

                outname = f"{epoch}_{base}.pkl"
                outfile = os.path.join(fk_out, outname)

                print("Saving motion to:", outfile)
                
               # Simply reshape the data - full_q is already rotation vectors!
                smpl_poses = full_q.squeeze(0).reshape((-1, 72)).cpu().numpy()  # (T, 72)
                smpl_trans = full_pos.squeeze(0).cpu().numpy()  # (T, 3)
